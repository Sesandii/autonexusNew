...
class ProfileController extends Controller
{
    ...
    public function updateProfile(): void
    {
        ...
        if (!$model->hasAgeColumn()) {
            $_SESSION['flash'] = 'Age field is not available in the database yet. Run database/add_age_to_users.sql first.';
        ...
        if ($ageRaw === '' || !ctype_digit($ageRaw)) {
            $_SESSION['flash'] = 'Age must be a valid number.';
            header('Location: ' . rtrim(BASE_URL,'/') . '/customer/profile/edit');
            exit;
        }
        ...
        $age = (int)$ageRaw;
        if ($age < 13 || $age > 120) {
            $_SESSION['flash'] = 'Age must be between 13 and 120.';
            header('Location: ' . rtrim(BASE_URL,'/') . '/customer/profile/edit');
            exit;
        }
...
class Profile
{
    ...
    /**
     * Check whether the users table has an age column.
     */
    public function hasAgeColumn(): bool
    {
        $userCols = $this->pdo->query("SHOW COLUMNS FROM users")->fetchAll(PDO::FETCH_COLUMN, 0);
        return in_array('age', $userCols, true);
    }
...
// edit.php - age input field(views/customer/profile/edit.php
  ...
  <div class="main-content customer-layout-main">
    ...
    <div class="profile-shell edit-shell">
      ...
      <form class="form-card edit-form" method="post" action="<?= $base ?>/customer/profile/update" enctype="multipart/form-data">
        ...
        <div class="grid edit-grid">
          <label>Age
            <input type="number" name="age" min="13" max="120" value="<?= htmlspecialchars((string)($profile['age'] ?? '')) ?>" required>
          </label>
          <label>State
...
//views/customer.profile/index.php
      ...
      <div class="profile-grid">
        <section class="panel info-panel">
          ...
          <div class="info-table">
            ...
            <div class="info-row"><span>Age</span><span><?= htmlspecialchars($age) ?></span></div>
