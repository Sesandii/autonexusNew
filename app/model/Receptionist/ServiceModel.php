    ...
    public function getAllPackages(): array
{
    $stmt = $this->db->prepare("
        SELECT package_id, package_code, name, description,
               total_duration_minutes, total_price, status
        FROM packages
    ...
    foreach ($packages as &$pkg) {
        $stmt = $this->db->prepare("
            SELECT s.service_id, s.service_code, s.name, s.description,
                   s.base_duration_minutes, s.default_price, s.status
            FROM services s
            INNER JOIN service_package_items spi
                ON s.service_id = spi.service_id
            WHERE spi.package_id = ?
              AND s.status = 'active'  -- Only active services in package
        ");
        $stmt->execute([$pkg['package_id']]);
        $pkg['services'] = $stmt->fetchAll(PDO::FETCH_ASSOC);
