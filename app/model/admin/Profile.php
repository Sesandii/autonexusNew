...
class Profile
{
    ...
    private function detectStreetColumn(): ?string
    {
        ...
        if (in_array('street', $cols, true))
            return 'street';
        if (in_array('street_address', $cols, true))
    ...
    public function getAdminById(int $userId): ?array
    {
        ...
        if ($row && !isset($row['street_address']) && isset($row['street'])) {
            $row['street_address'] = $row['street'];
        }
        if ($row && !isset($row['street']) && isset($row['street_address'])) {
            $row['street'] = $row['street_address'];
        }
        return $row ?: null;
    }

    // Handle updateAdmin operation.
    public function updateAdmin(int $userId, array $data): bool
    {
        try {
