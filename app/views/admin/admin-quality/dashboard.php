...
<head>
  ...
  <style>
    ...
    .list li{display:flex;justify-content:space-between;gap:14px;padding:10px 0;border-bottom:1px solid #e2e8f0}
...
<main class="main-content">
  ...
  <p class="muted">Deeper quality-control analytics using existing reports, checklists, final reports, and photo evidence.</p>
  ...
  <section class="cards">
    ...
    <div class="mini"><div class="label">Low Rated Cases</div><div class="value"><?= (int)($summary['low_rated_total'] ?? 0) ?></div></div>
    <div class="mini"><div class="label">Checklist Failures</div><div class="value"><?= (int)($summary['failed_checklist_total'] ?? 0) ?></div></div>
  </section>

  <section class="cards" style="margin-top:0;">
    <div class="mini"><div class="label">Reinspection Queue</div><div class="value"><?= (int)($summary['reinspection_queue_total'] ?? 0) ?></div></div>
    <div class="mini"><div class="label">Checklist Compliance</div><div class="value"><?= number_format((float)$checklistCompliance, 1) ?>%</div></div>
    <div class="mini"><div class="label">Test Drive Completion</div><div class="value"><?= number_format((float)$testDriveRate, 1) ?>%</div></div>
    <div class="mini"><div class="label">Concerns Addressed</div><div class="value"><?= number_format((float)$concernRate, 1) ?>%</div></div>
  </section>
        ...
        <?php if (!empty($failedCases)): ?>
          <?php foreach ($failedCases as $r): ?>
            <li style="display:block;">
              ...
              <div class="muted"><?= htmlspecialchars($r['customer_name'] ?? '—') ?> • <?= htmlspecialchars($r['branch_name'] ?? '—') ?> • <?= htmlspecialchars($r['service_name'] ?? '—') ?></div>
              <div class="muted">Rating: <?= htmlspecialchars((string)($r['quality_rating'] ?? '0')) ?> | Checklist: <?= (int)($r['checklist_verified'] ?? 0) ? 'Yes' : 'No' ?> | Test Drive: <?= (int)($r['test_driven'] ?? 0) ? 'Yes' : 'No' ?> | Concerns: <?= (int)($r['concerns_addressed'] ?? 0) ? 'Yes' : 'No' ?></div>
            </li>
          <?php endforeach; ?>
        <?php else: ?>
          ...
          <?php foreach ($reinspectionQueue as $r): ?>
            <li style="display:block;">
              <div style="display:flex;justify-content:space-between;gap:12px;">
                ...
                ?>
                <span class="badge <?= $severity ?>"><?= htmlspecialchars(ucfirst((string)($r['status'] ?? 'draft'))) ?></span>
              </div>
              <div class="muted"><?= htmlspecialchars($r['customer_name'] ?? '—') ?> • <?= htmlspecialchars($r['branch_name'] ?? '—') ?></div>
              <div class="muted">Rating: <?= htmlspecialchars((string)($r['quality_rating'] ?? '0')) ?> | Checklist: <?= (int)($r['checklist_verified'] ?? 0) ? 'Yes' : 'No' ?> | Test Drive: <?= (int)($r['test_driven'] ?? 0) ? 'Yes' : 'No' ?> | Concerns: <?= (int)($r['concerns_addressed'] ?? 0) ? 'Yes' : 'No' ?></div>
