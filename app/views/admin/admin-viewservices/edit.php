...
<head>
  ...
  <style>
    ...
    .branch-box{border:1px solid #e2e8f0;border-radius:12px;padding:12px;max-height:280px;overflow:auto;background:#f8fafc}
  ...
  <form method="post" action="<?= $base ?>/admin/services/<?= (int)$row['service_id'] ?>">
    <div class="card">
      <div class="grid">
        ...
        <div class="field">
          ...
          <input type="number" id="base_duration_minutes" name="base_duration_minutes" min="0" value="<?= (int)$row['base_duration_minutes'] ?>">
        </div>

        <div class="field">
          <label for="default_price">Price</label>
          <input type="number" id="default_price" name="default_price" min="0" step="0.01" value="<?= htmlspecialchars((string)$row['default_price']) ?>">
        </div>
      </div>
    ...
    <div class="card">
      ...
      <div class="field" style="margin-bottom:12px;">
        <label><input type="radio" name="apply_scope" value="all" <?= !empty($applyAll) ? 'checked' : '' ?>> Apply to all active branches</label>
        <label><input type="radio" name="apply_scope" value="specific" <?= empty($applyAll) ? 'checked' : '' ?>> Apply to specific branches</label>
      </div>

      <div id="branchPicker" class="branch-box" style="display: <?= !empty($applyAll) ? 'none' : 'block' ?>;">
        <?php
