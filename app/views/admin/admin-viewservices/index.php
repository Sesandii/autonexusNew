...
<head>
  ...
  <style>
    ...
    .muted{color:#64748b}
    .btn{display:inline-flex;align-items:center;gap:8px;padding:10px 14px;border-radius:10px;text-decoration:none;border:0;cursor:pointer}
    .btn-primary{background:#dc2626;color:#fff}
    .btn-primary:hover{background:#ef4444}
    .section-card{background:#fff;border:1px solid #e2e8f0;border-radius:16px;padding:20px;margin-bottom:22px}
    .section-card h2{margin:0 0 6px}
    .tabs{display:flex;gap:8px;flex-wrap:wrap;margin-top:14px}
    .tab{border:1px solid #cbd5e1;background:#fff;border-radius:999px;padding:8px 14px;cursor:pointer}
    .tab.active{background:#111827;color:#fff;border-color:#111827}
    .table-wrap{overflow:auto;border:1px solid #e2e8f0;border-radius:14px;margin-top:16px}
    ...
    .icon-link,.icon-btn{background:none;border:0;cursor:pointer;color:#111827}
    .pkg-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(320px,1fr));gap:16px;margin-top:16px}
    .pkg-card{background:#fff;border:1px solid #e2e8f0;border-radius:16px;padding:18px}
    .pkg-top{display:flex;justify-content:space-between;gap:12px;align-items:flex-start}
    .pkg-top h3{margin:0}
    .pkg-meta{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px;margin:14px 0}
    .metric{background:#f8fafc;border:1px solid #e2e8f0;border-radius:12px;padding:10px}
    .metric .label{font-size:12px;color:#64748b}
    .metric .value{font-weight:700;margin-top:3px}
    .pkg-items{margin:12px 0 0;padding-left:18px}
    .pkg-items li{margin-bottom:6px}
...
<main class="main-content">
  <div class="page-head">
    ...
    <a class="btn btn-primary" href="<?= $base ?>/admin/services/create">
      <i class="fa-solid fa-plus"></i> Add New Service / Package
    </a>
  </div>
        ...
        <?php if (!empty($services)): ?>
          <?php foreach ($services as $row): ?>
            <tr data-type="type-<?= (int)($row['type_id'] ?? 0) ?>">
              ...
              <td><span class="badge badge-type"><?= htmlspecialchars($row['type_name'] ?? 'Uncategorized') ?></span></td>
              ...
              <td class="actions">
                <a class="icon-link" href="<?= $base ?>/admin/services/<?= (int)$row['service_id'] ?>/edit" title="Edit">
                  <i class="fa-solid fa-pen"></i>
                </a>
                <form method="post" action="<?= $base ?>/admin/services/<?= (int)$row['service_id'] ?>/delete" onsubmit="return confirm('Delete this service?');">
          ...
          <?php endforeach; ?>
        <?php else: ?>
          <tr><td colspan="8" class="empty">No services found.</td></tr>
        <?php endif; ?>
        </tbody>
      </table>
    </div>
          ...
          <div class="pkg-card">
            <div class="pkg-top">
              <div>
                ...
                <div class="small"><?= htmlspecialchars($pkg['service_code']) ?> · <?= htmlspecialchars($pkg['type_name']) ?></div>
              </div>
            ...
            <div class="actions" style="margin-top:14px;">
              <a class="icon-link" href="<?= $base ?>/admin/services/<?= (int)$pkg['service_id'] ?>/edit" title="Edit">
                <i class="fa-solid fa-pen"></i> Edit
              </a>
              <form method="post" action="<?= $base ?>/admin/services/<?= (int)$pkg['service_id'] ?>/delete" onsubmit="return confirm('Delete this package?');">
                <button type="submit" class="icon-btn" title="Delete">
