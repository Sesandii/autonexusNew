...
<body>
    ...
    <table>
        <tr>
            <th>Vehicle</th>
            <td><?= htmlspecialchars($workOrder['license_plate']) ?></td>
            <th>Service</th>
            <td><?= htmlspecialchars($workOrder['service_name']) ?></td>
        </tr>
        <tr>
            <th>Customer</th>
            <td><?= htmlspecialchars($workOrder['customer_first_name'] . ' ' . $workOrder['customer_last_name']) ?></td>
            <th>Mechanic</th>
            <td><?= htmlspecialchars($workOrder['mechanic_code']) ?></td>
        </tr>
    </table>
    ...
    <table style="width: 100%; border: none;">
        <tr>
            ...
            <td style="width: 50%; border: none; vertical-align: top;">
                ...
                <div class="checklist-item">
                    <span class="check-box"><?= $report['concerns_addressed'] ? '✓' : '' ?></span> Customer concerns
                    addressed
                </div>
            </td>
        </tr>
    ...
    <table class="continuity-table">
        <tr>
            <td style="padding: 15px;">
                ...
                <?= htmlspecialchars($workOrder['last_service_mileage'] ?? 'Not set') ?> km
            </td>
            <td style="padding: 15px;">
                <strong>Service Interval:</strong> <?= htmlspecialchars($workOrder['service_interval_km'] ?? '5000') ?>
                km
            </td>
        </tr>
    </table>
    ...
    <?php if (empty($includeImages)): ?>
        <p style="color: #666; font-size: 11px;">Photos were omitted because PHP GD extension is not enabled on the server.
        </p>
