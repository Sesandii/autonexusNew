...
class VehicleReportsController extends Controller
{
    ...
    private function generateMechanicPDF($activity, $date, $chart1 = null, $chart2 = null)
    {
        $options = new \Dompdf\Options();
        $options->set('isHtml5ParserEnabled', true);
        $options->set('isRemoteEnabled', true);
        ...
        $chart1Data = $chart1 ?? ($_POST['chart1_data'] ?? null);
        $chart2Data = $chart2 ?? ($_POST['chart2_data'] ?? null);

        if (!$gdAvailable) {
            // Dompdf needs GD for PNG/chart rendering; continue export without chart images.
            $chart1Data = null;
            $chart2Data = null;
        }
    ...
    public function exportDaily()
    {
        ...
        if ($format === 'csv') {
            ...
            if (!$gdAvailable) {
                // Dompdf needs GD for PNG/chart rendering; keep export working without charts.
                $chartStatus = null;
                $chartHourly = null;
                $chartTrend = null;
            }
